...
export interface OtelMetric {
  /** Full metric name, e.g. code_review_toolkit.tokens.input */
  name: string;
  value: number;
  /** Datapoint attributes — the OTel equivalent of Datadog tags (repo, model, verdict, …) */
...
export function buildResourceAttributes(extra?: Record<string, string>): Array<{ key: string; value: OtlpAttributeValue }> {
...
export function buildOtlpRequest(
  metrics: OtelMetric[],
  resourceAttrs: Record<string, string> | undefined,
  tsMs: number,
): ExportMetricsServiceRequest {
  const tsNano = (BigInt(Math.floor(tsMs)) * 1_000_000n).toString();
  return {
    resourceMetrics: [{
...
export interface OtlpPushResult {
  ok: boolean;
  status: number;
  /** Set when the server accepted the request partially (ExportMetricsServiceResponse) */
  partial?: { rejectedDataPoints: number; errorMessage?: string };
  error?: string;
}

/** Retryable per the OTLP/HTTP spec: 429, 502, 503, 504. Everything else must not be retried. */
export function isRetryableOtlpStatus(status: number): boolean {
  return status === 429 || status === 502 || status === 503 || status === 504;
}

export async function pushToOtlp(
  endpoint: string,
  authHeader: string,
  ...
  try {
    ...
    const result: OtlpPushResult = { ok: resp.ok, status: resp.status };
    if (resp.ok) {
      try {
        const text = await resp.text();
        if (text) {
          const parsed = JSON.parse(text) as {
            partialSuccess?: { rejectedDataPoints?: number | string; errorMessage?: string };
          };
 ...
 */
export function readOtlpConfigFromEnv(): { endpoint: string; authHeader: string; timeoutMs: number } | null {
