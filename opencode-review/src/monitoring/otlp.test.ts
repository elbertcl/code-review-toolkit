...
describe("buildOtlpRequest", () => {
  it("wraps metrics in a valid ExportMetricsServiceRequest shape", () => {
    ...
    const attrs = Object.fromEntries(
      (first.gauge.dataPoints[0].attributes ?? []).map((a) => [a.key, a.value.stringValue]),
    );
  ...
  it("applies extra resource attributes alongside service.name", () => {
    ...
    const attrs = Object.fromEntries(
      body.resourceMetrics[0].resource.attributes.map((a) => [a.key, a.value.stringValue]),
    );
...
describe("isRetryableOtlpStatus", () => {
  it("classifies per the OTLP/HTTP spec", () => {
    for (const retryable of [429, 502, 503, 504]) assert.ok(isRetryableOtlpStatus(retryable), `${retryable} retryable`);
    for (const fatal of [400, 401, 403, 404, 413, 500]) assert.ok(!isRetryableOtlpStatus(fatal), `${fatal} not retryable`);
  });
});
...
describe("buildResourceAttributes", () => {
  it("always includes service.name and cannot be overridden by extras", () => {
    const attrs = Object.fromEntries(
      buildResourceAttributes({ "service.name": "evil" }).map((a) => [a.key, a.value.stringValue]),
    );
    assert.equal(attrs["service.name"], "code-review-toolkit");
  });
});
