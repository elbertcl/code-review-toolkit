...
export function buildRunMetrics(row) {
    const attributes = {};
    if (row.repo)
        attributes.repo = row.repo;
    if (row.model)
    ...
    if (row.mode)
        attributes.mode = row.mode;
    if (row.verdict)
        attributes.verdict = row.verdict;
    const points = [];
    const t = row.tokens ?? {};
    if (Number(t.input))
...
export async function pushRunMetrics(config, row) {
...
if (process.argv[1] && process.argv[1].endsWith("push-run-metrics.js")) {
    ...
    try {
        ...
        if (!result.ok)
            process.stdout.write(`metrics: push failed (HTTP ${result.status}${result.error ? `: ${result.error.slice(0, 200)}` : ""})\n`);
        else if (result.partial)
            process.stdout.write(`metrics: partial success (${result.partial.rejectedDataPoints} rejected: ${result.partial.errorMessage ?? "no reason"})\n`);
        else
