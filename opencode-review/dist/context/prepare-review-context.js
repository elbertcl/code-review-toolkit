...
import { GAP_MARKER, ORGANIZATION_PROFILE_ALLOWLIST, parseManifest, selectConditionalContext, validateManifest, } from "./lib/review-manifest.js";
...
export function readAtRef(workspace, trustedRef, declaredPath) {
    ...
    if (!mode)
        throw new Error(`${declaredPath} is missing at trusted ref`);
    if (mode === "120000")
        throw new Error(`${declaredPath} is a symlink at trusted ref`);
    if (mode !== "100644" && mode !== "100755")
        throw new Error(`${declaredPath} is not a regular file at trusted ref`);
    try {
...
function assertPolicyDoesNotContainMandatoryRules(policy, ruleIds) {
    for (const ruleId of ruleIds) {
        if (policy.includes(ruleId))
            throw new Error(`Repository policy must not contain mandatory organization rule ID ${ruleId}`);
    }
}
...
export function buildOpenThreadsSection(openThreads) {
    if (!Array.isArray(openThreads) || openThreads.length === 0)
        return null;
    const directive = "The following reviewer threads are already open on this PR. Do NOT re-flag " +
...
function assertTrustedCommit(workspace, trustedRef) {
    if (!/^[0-9a-f]{40}$/.test(trustedRef))
        throw new Error("trustedRef must be a full 40-character commit SHA");
    try {
...
export async function compileReviewContext({ workspace, trustedRef, changedFiles = [], orgContextsDir, outputPath, maxBytes = 500_000, openThreadsPath, }) {
    ...
    const addSource = (sourcePath, content) => {
        ...
        if (sourceBytes > maxBytes)
            throw new Error(`Compiled review context exceeds ${maxBytes} bytes while adding ${sourcePath}`);
        sources.push(source);
    };
    ...
    if (new Set(mandatoryRuleIds).size !== mandatoryRuleIds.length)
        throw new Error("Organization profiles contain duplicate mandatory rule IDs");
    ...
    const blockers = [];
    if (manifestText.includes(GAP_MARKER))
        blockers.push(`${MANIFEST_PATH} is incomplete`);
    if (policy.includes(GAP_MARKER))
        blockers.push(`${manifest.policy_path} is incomplete`);
    else
        addSource(manifest.policy_path, policy);
    for (const entry of requiredEntries) {
        try {
            const content = readAtRef(workspace, trustedRef, entry.path);
            if (content.includes(GAP_MARKER))
                blockers.push(`${entry.path} is incomplete`);
            else
                addSource(entry.path, content);
        }
        catch (error) {
    ...
    if (openThreadsPath) {
        ...
        if (threadsSection) {
            ...
            if (sourceBytes > maxBytes)
                throw new Error(`Compiled review context exceeds ${maxBytes} bytes while adding pr/open-threads`);
...
async function main() {
    ...
    if (!Array.isArray(changedFiles) || changedFiles.some((value) => typeof value !== "string"))
        throw new Error("--changed-files must contain a JSON string array");
...
//# sourceMappingURL=prepare-review-context.js.map