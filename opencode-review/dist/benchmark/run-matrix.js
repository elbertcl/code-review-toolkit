...
import { evaluateCell } from "./evaluate.js";
...
function runOcr(model, entry, distRoot, defaultsDir) {
...
function main() {
    ...
    if (process.env.OCR_LLM_URL)
        sh("ocr", ["config", "set", "llm.url", process.env.OCR_LLM_URL]);
    if (process.env.OCR_LLM_TOKEN)
        sh("ocr", ["config", "set", "llm.auth_token", process.env.OCR_LLM_TOKEN]);
        ...
        for (const entry of corpus.entries) {
            for (let repeat = 1; repeat <= repeats; repeat++) {
                try {
                    ...
                    cells.push({ model, entryId: entry.id, repeat, ok: false, error: error.message, precision: null, recall: null, severity_match_rate: null, rule_citation_rate: null });
                    process.stdout.write(`ERROR ${model} ${entry.id} #${repeat}: ${error.message}\n`);
                }
            }
        }
    }
