...
export function allocateBudget(threads, cap) {
    if (threads.length === 0)
        return [];
    const perThread = Math.floor(cap / threads.length);
    return threads.map(() => perThread);
}
function formatDigest(thread, budget) {
    ...
    if (Buffer.byteLength(combined) <= available)
        return `${header} ${combined}`;
    let truncated = "";
    let bytes = 0;
    for (const char of combined) {
        const charBytes = Buffer.byteLength(char);
        if (bytes + charBytes > available - 14)
            break;
        truncated += char;
        bytes += charBytes;
    }
...
export function buildBackground(threads, serenaContext) {
    ...
    if (serenaContext) {
        ...
        if (serenaBytes >= BUDGET_WITH_SERENA - 20)
            return serenaContext;
        ...
        const resolvedHeaders = resolved.map((t) => `- ${t.path}:${t.line} (resolved, ${t.comment_count} comment${t.comment_count !== 1 ? "s" : ""}):`);
...
if (process.argv[1] && process.argv[1].endsWith("build-background.js")) {
    ...
    if (serenaPath) {
        try {
            serenaContext = readFileSync(serenaPath, "utf8");
        }
        catch { /* fail-open */ }
    }
