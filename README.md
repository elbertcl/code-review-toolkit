...
Consuming repos carry only three artifacts:
- a ~15-line workflow calling `opencode-review@v4`
- a root `REVIEW.md` manifest (generated by the `initialize-review-context` skill)
- a `.serena/` project index (generated once by the skill)

Serena MCP is enabled by default (`serena: true`) and fails open if unavailable.
Set `serena: false` to disable.
