...
export async function compileReviewContext({ workspace, trustedRef, changedFiles = [], orgContextsRoot, outputPath, maxBytes = 500_000 }) {
  ...
  for (const profile of manifest.organization_profiles) {
    ...
    const absolutePath = path.resolve(orgContextsRoot, relativePath);
    const root = path.resolve(orgContextsRoot);
    if (!absolutePath.startsWith(`${root}${path.sep}`)) throw new Error(`Organization profile ${profile} escapes its root`);
    const stat = await lstat(absolutePath);
    if (stat.isSymbolicLink() || !stat.isFile()) throw new Error(`Organization profile ${profile} must be a regular file`);
...
async function main() {
  ...
  const result = await compileReviewContext({
    ...
    changedFiles,
    orgContextsRoot: options["org-contexts"],
    outputPath,
