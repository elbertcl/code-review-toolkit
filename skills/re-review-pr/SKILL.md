...
Replace `{HEAD_SHA}` with the full `HEAD_SHA` value from Phase 2.4. This marker is an invisible HTML comment that the re-review pre-flight uses to detect whether a complete review cycle has been posted for the current HEAD — required to distinguish a finished re-review from a run that posted inline comments but timed out before emitting the verdict.
